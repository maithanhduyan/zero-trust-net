...
Xóa node cũ và đăng ký lại với IP mới:
...
Database sạch. Bây giờ restart agent để đăng ký lại:
...
## ✅ Agent đã đăng ký với IP đúng: **10.10.0.2/24**

Bây giờ update peer trên Hub và test connectivity:
...
Ran terminal command: ping -c 3 10.10.0.2 && echo "✅ VPN TUNNEL TO AGENT WORKING!"

Vẫn chưa ping được. Kiểm tra config trên agent:

Ran terminal command: ssh root@155.xxx.xxx.254 'ip addr show wg0; echo "---"; cat /etc/wireguard/wg0.conf; echo "---"; ping -c 2 10.10.0.1'

Có vấn đề:
1. **wg0 interface** có IP `10.10.0.100/24` (từ config cũ) nhưng **config file** là `10.10.0.2/24`
2. **Endpoint** trong config vẫn dùng **IPv6** `2400:d320:2261:7974::1:51820` thay vì IPv4
...
Agent ping được Hub! Bây giờ test từ Hub:

Ran terminal command: ping -c 3 10.10.0.2 && echo "✅ BIDIRECTIONAL VPN TUNNEL WORKING!"

## 🎉 AGENT INSTALLATION TEST SUCCESSFUL!

Tóm tắt kết quả:

| Component | Status | Details |
|-----------|--------|---------|
| Hub → Agent | ✅ | ping 10.10.0.2 OK |
| Agent → Hub | ✅ | ping 10.10.0.1 OK |
| Agent Registration | ✅ | IP: 10.10.0.2/24 |
| Agent Service | ✅ | Running, sync loop active |
| WireGuard Tunnel | ✅ | Handshake complete |

Cần fix script để dùng IPv4 endpoint thay vì IPv6. Commit và push changes:

Ran terminal command: cd /home/zero-trust-netwoking && git add -A && git commit -m "fix: Update config to use 10.10.0.0/24 network and IPv4 endpoint" && git push

## ✅ Test Cài Đặt Agent Hoàn Tất!

### Kết quả test trên node 155.xxx.xxx.254:

| Giai đoạn | Kết quả |
|-----------|---------|
| Phase 0: Kiểm tra yêu cầu | ✅ Pass |
| Phase 1: Dependencies | ✅ WireGuard, Git đã sẵn sàng |
| Phase 2: WireGuard Keypair | ✅ Key generated |
| Phase 3: Clone Agent Code | ✅ `/opt/zero-trust-agent` |
| Phase 4: Đăng ký | ✅ IP: **10.10.0.2/24** |
| Phase 5: WireGuard Config | ✅ Cấu hình & khởi động |
| Phase 6: Hướng dẫn Hub | ℹ️ Manual step shown |
| Phase 7: Agent Service | ✅ Running |
| Phase 8: Verify | ✅ VPN connected |

### Network Status:
```
Hub (10.10.0.1) ←──── WireGuard Tunnel ────→ Agent (10.10.0.2)
    │                                              │
    └─── 5.104.82.252:51820 ◄─── 155.xxx.xxx.254 ───┘
```

### Issues Fixed:
1. Branch `main` → `master`
2. Config network `10.0.0.0/24` → `10.10.0.0/24`
3. Endpoint IPv6 → IPv4
